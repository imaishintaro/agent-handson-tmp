...
const ATTACHMENTS_DIR = (0, path_1.join)(WORK_DIR, ".discord-attachments");
// セッションマネージャーの初期化
const sessionManager = new claude_session_1.ClaudeSessionManager(WORK_DIR, DEFAULT_MODEL);
// Discordクライアントの初期化
 ...
 */
function extractAttachableFiles(text, workDir) {
    ...
    for (const filePath of filePaths) {
        const absolutePath = (0, path_1.resolve)(workDir, filePath);
        try {
            ...
            if (!stats.isFile())
                continue;
            if (stats.size > DISCORD_FILE_SIZE_LIMIT)
                continue;
            if (stats.size === 0)
                continue;
...
async function downloadAttachment(url, filename) {
    ...
    if (!(0, fs_1.existsSync)(ATTACHMENTS_DIR)) {
        (0, fs_1.mkdirSync)(ATTACHMENTS_DIR, { recursive: true });
    }
    // ファイル名の衝突を避けるためタイムスタンプを付加
    const timestamp = Date.now();
    const safeName = filename.replace(/[^a-zA-Z0-9._-]/g, "_");
    const localPath = (0, path_1.join)(ATTACHMENTS_DIR, `${timestamp}_${safeName}`);
    const response = await fetch(url);
    if (!response.ok || !response.body) {
        throw new Error(`ダウンロード失敗: ${response.statusText}`);
    }
    // Node.js ReadableStreamをファイルに書き込む
    const fileStream = (0, fs_1.createWriteStream)(localPath);
    await (0, promises_1.pipeline)(stream_1.Readable.fromWeb(response.body), fileStream);
    return localPath;
}
/**
...
async function processAttachments(attachments) {
    ...
    for (const [, attachment] of attachments) {
        try {
            ...
            if (isImage) {
                promptParts.push(`[添付画像: ${attachment.name}] → ${localPath}`);
            }
            else {
                promptParts.push(`[添付ファイル: ${attachment.name}] → ${localPath}\nこのファイルの内容を読んで処理してください。`);
            }
        }
        catch (error) {
            const errorMsg = error instanceof Error ? error.message : "不明なエラー";
            promptParts.push(`[添付ファイル: ${attachment.name}] ダウンロード失敗: ${errorMsg}`);
        }
    }
    const promptAddition = promptParts.length > 0
        ? "\n\n--- 添付ファイル ---\n" + promptParts.join("\n")
        : "";
    return { promptAddition, downloadedPaths };
}
    ...
    return (event) => {
        switch (event.type) {
            ...
            case "task_started":
                ...
                break;
            case "task_completed":
                logLines.push(event.status === "completed"
                    ? `✅ 完了: ${event.summary}`
 ...
 * ユーザーのメッセージ受信をログ出力
 */
function logUserMessage(username, userId, channelInfo, prompt) {
...
function logProgress(event) {
    switch (event.type) {
        ...
        case "tool_progress":
            log("🔧 実行中", C.yellow, `${toolDisplayName(event.toolName)} (${Math.floor(event.elapsedSeconds)}秒経過)`);
            break;
        case "tool_summary":
        ...
        case "task_completed": {
            ...
            log(`${icon} タスク完了`, event.status === "completed" ? C.green : C.red, `[${event.status}] ${event.summary}`);
            break;
        }
    }
}
 ...
 * Claudeの応答をログ出力
 */
function logResponse(responseText, costUsd) {
 ...
 */
async function sendResponse(responseText, costUsd, channelId, editFirstMessage, sendToChannel, sendFilesToChannel) {
    ...
    const attachments = attachableFiles.map((filePath) => new discord_js_1.AttachmentBuilder(filePath, { name: (0, path_1.basename)(filePath) }));
    ...
    const attachInfo = attachments.length > 0
        ? `\n-# 添付: ${attachableFiles.map((f) => (0, path_1.basename)(f)).join(", ")}`
        : "";
    const footer = attachInfo + wsInfo + costInfo;
    // 最初のチャンクで元のメッセージを編集
    await editFirstMessage(chunks[0] + (chunks.length === 1 ? footer : ""));
    // 残りのチャンクを追加送信
...
client.once("ready", () => {
    ...
    console.log(`許可ユーザー: ${ALLOWED_USER_IDS.length === 0 ? "全員" : ALLOWED_USER_IDS.join(", ")}`);
    // システムプロンプトの読み込み元を表示
    const identifyMdPath = `${WORK_DIR}/identify.md`;
    const contextMdPath = `${WORK_DIR}/context.md`;
    const hasIdentify = (0, fs_1.existsSync)(identifyMdPath);
    const hasContext = (0, fs_1.existsSync)(contextMdPath);
    if (hasIdentify || hasContext) {
        if (hasIdentify)
            console.log(`システムプロンプト: identify.md (性格・口調)`);
        if (hasContext)
            console.log(`システムプロンプト: context.md  (プロジェクト情報)`);
    }
...
client.on("interactionCreate", async (interaction) => {
    ...
    if (commandName === "claude-model") {
        ...
        await interaction.reply(`モデルを **${displayName}** に変更しました。\n次のメッセージからこのモデルが使用されます。`);
        return;
    }
    ...
    if (commandName === "claude-workspace") {
        ...
        try {
            ...
            const existingChannels = guild.channels.cache.filter((ch) => ch.parentId === category.id);
            ...
            await interaction.editReply(`ワークスペース **${name}** を登録しました。\n` +
                `📁 ディレクトリ: \`${directory}\`\n` +
                `📂 カテゴリ: ${category.name}\n\n` +
    ...
    if (commandName === "claude-workspaces") {
        ...
        if (workspaces.length === 0) {
            await interaction.reply({
                content: "登録されたワークスペースはありません。\n`/claude-workspace` で登録してください。",
                ephemeral: true,
            });
            return;
        }
        const list = workspaces
            .map((ws) => `• **${ws.name}** → \`${ws.directory}\``)
            .join("\n");
        await interaction.reply({
            content: `**ワークスペース一覧:**\n${list}`,
            ephemeral: true,
    ...
    if (commandName === "claude") {
        ...
        if (channel && "parentId" in channel && channel.parentId) {
            sessionManager.setChannelCategory(interaction.channelId, channel.parentId);
        }
        // ユーザーメッセージをコンソールに出力
        ...
        const onProgress = createProgressUpdater((content) => interaction.editReply(content));
        const onProgressWithLog = (event) => {
            logProgress(event);
            onProgress(event);
        };
        try {
            const { result, costUsd } = await sessionManager.sendPrompt(interaction.channelId, prompt, onProgressWithLog);
            const responseText = result || "（応答なし）";
            logResponse(responseText, costUsd);
            await sendResponse(responseText, costUsd, interaction.channelId, (content) => interaction.editReply(content), (content) => interaction.channel.send(content), (files) => interaction.channel.send({ files }));
        }
        catch (error) {
...
client.on("messageCreate", async (message) => {
    ...
    if (!prompt && message.attachments.size === 0) {
        await message.reply(`メッセージを入力してください。例: \`${PREFIX} このプロジェクトの構成を教えて\``);
        return;
    }
    // チャンネルの親カテゴリ情報をキャッシュ
    if ("parentId" in message.channel && message.channel.parentId) {
        sessionManager.setChannelCategory(message.channelId, message.channel.parentId);
    }
    // ユーザーメッセージをコンソールに出力
    ...
    // 進捗更新用コールバック（Discord表示 + コンソールログ）
    const onProgress = createProgressUpdater((content) => thinkingMessage.edit(content));
    const onProgressWithLog = (event) => {
        logProgress(event);
        onProgress(event);
    };
    try {
        ...
        const { result, costUsd } = await sessionManager.sendPrompt(message.channelId, fullPrompt, onProgressWithLog);
        const responseText = result || "（応答なし）";
        logResponse(responseText, costUsd);
        const channel = message.channel;
        await sendResponse(responseText, costUsd, message.channelId, (content) => thinkingMessage.edit(content), (content) => channel.send(content), (files) => channel.send({ files }));
    }
    catch (error) {
        const errorMessage = error instanceof Error ? error.message : "不明なエラー";
        log("❌ エラー", C.red, errorMessage);
